...
class PickAruco(object):
	...
	def pick_aruco(self, string_operation,req):
		...
		rospy.sleep(2.0)
		rospy.loginfo("spherical_grasp_gui: Waiting for an aruco detection Matts version")
		...
		if string_operation == "pick":
			...
			pick_g.object_pose.pose.position = aruco_ps.pose.position
                        pick_g.object_pose.pose.position.z -= 0.125#0.1*(1.0/2.0)
			#pick_g.object_pose.pose.position.z = 0.18
			...
			#pick_g.object_pose.pose.position.z += 0.12
	...
	def prepare_robot(self):
		...
		rospy.loginfo("Robot prepared. Matts version")
